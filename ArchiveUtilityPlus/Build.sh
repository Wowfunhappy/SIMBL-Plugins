...
PLUGIN_NAME="ArchiveUtilityPlus"
VERSION=$(date +%Y.%m.%d)
...
# Copy Frameworks directory contents if it exists
if [ -d "./Frameworks" ]; then
    echo "Copying Frameworks..."
    cp -R ./Frameworks/* "${BUNDLE_DIR}/Contents/Frameworks/"
fi
...
# Register the companion icon app with Launch Services
INSTALLED_ICON_APP="${INSTALL_DIR}/${PLUGIN_NAME}.bundle/Contents/Resources/${ICON_APP}"
if [ -d "${INSTALLED_ICON_APP}" ]; then
    echo "Registering icon app with Launch Services..."
    /System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister -f "${INSTALLED_ICON_APP}"
fi
